...
const consentCheckboxId = 'client-review-consent-checkbox'
  ...
  return (
    <Card className="border-dashed animate-in">
      <CardContent className="flex flex-col items-center justify-center py-16 px-6 text-center">
        ...
        <h2 className="text-h3 font-semibold text-foreground mt-4">
          No options available
        </h2>
        <p className="text-body text-muted-foreground mt-2 max-w-sm">
          There are no options to select at this time. Please check back later or
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {mockOptions.length === 0 ? (
            ...
            mockOptions.map((option) => (
              <Card
                key={option.id}
                className={`cursor-pointer transition-all duration-300 bg-card ${
                  selected === option.id
        ...
        <div className="mt-12 space-y-4">
          <div className="flex items-start gap-3">
            <Checkbox
              ...
              aria-label="I confirm my selection and consent to this approval"
              className="mt-0.5"
            />
            <Label
              htmlFor={consentCheckboxId}
              className="text-body cursor-pointer font-normal leading-6 peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
            >
          ...
          <Button
            size="lg"
            disabled={!selected || !consentChecked}
